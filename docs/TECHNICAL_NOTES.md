...
- **Экран клиента:** displays only the most recently published confirmed/paid vehicle and follows backend updates, including in a separate `/?page=client` window. Selecting or editing an unconfirmed row does not publish it. Editing or rejecting the displayed record clears it until it is confirmed again.
...
.venv/Scripts/python.exe -m uvicorn web_app.main:app --host 127.0.0.1 --port 8000
```
